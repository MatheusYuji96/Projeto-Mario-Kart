...
primary key (ceTitulos, ceVeiculos, idTipos),
foreign key (ceTitulos) references Títulos(idTitulos),
foreign key (ceVeiculos) references Veículos(idVeiculos));
...
constraint chkCaracteristica
check (Característica in ('Defesa', 'Ataque', 'Velocidade', 'Outros')),
primary key (ceTitulos, ceItens, idUtilidade),
foreign key (ceTitulos) references Títulos(idTitulos),
foreign key (ceItens) references Itens(idItens));
